...
import UIKit
import SwiftSoup
class AqggTableViewController: UITableViewController {
    var aqgg:[TableCell]=[]
    let base_url:String="https://itsc.nju.edu.cn/aqtg/list"
...
//                    return self.max_page
        ...
        do {
            ...
            for i in content{
                if(count1<=content.count-12){
                    ...
                    print(t,d,linkhref)
                    let temp:TableCell=TableCell(t: String(t.prefix(25))+"...",d: String(d),w: linkhref)
                    self.aqgg.append(temp)
                }
                count1+=1
            }
            //print(content)
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = tableView.dequeueReusableCell(withIdentifier: "AqggCell", for: indexPath) as! AqggTableViewCell
        if(indexPath.row >= self.aqgg.count){
            cell.title.text! = ""
            cell.date.text! = ""
            return cell
        }
        let news = self.aqgg[indexPath.row]
        // Configure the cell...
    ...
    */
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        let cell = sender as! AqggTableViewCell
        NewsViewController.myurl = aqgg[tableView.indexPath(for: cell)!.row].website
